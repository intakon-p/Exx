...
keypoint_colors = {
    ...
    Pose.LEFT_PINKY: 'white',#17
    Pose.RIGHT_PINKY: 'white',#18
    Pose.LEFT_INDEX: 'white',#19
    Pose.RIGHT_INDEX: 'white',#20
    Pose.LEFT_THUMB: 'white',#21
    Pose.RIGHT_THUMB: 'white',#22
    ...
    Pose.LEFT_HEEL: 'white',#29
    Pose.RIGHT_HEEL: 'white',#30
    Pose.LEFT_FOOT_INDEX: 'white',#31
    Pose.RIGHT_FOOT_INDEX: 'white',#32
...
# Create a new figure with subplots for top view, front view, and side view
fig, axs = plt.subplots(1, 3, figsize=(15, 5), subplot_kw={'projection': '3d'})
views = ['Top View', 'Front View', 'Side View']
view_angles = [(0, 0), (0, -90), (90, 0)]

for ax, view, angle in zip(axs, views, view_angles):
    ax.set_title(view)
    ax.view_init(elev=angle[0], azim=angle[1])  # Set viewing angle
...
while True:
    ...
    if results.pose_landmarks:
        ...
        # Adjust axis limits to fit all keypoints properly
        min_x = min(point[0] for point in keypoints_3d)
        max_x = max(point[0] for point in keypoints_3d)
        min_y = min(point[1] for point in keypoints_3d)
        max_y = max(point[1] for point in keypoints_3d)
        min_z = min(point[2] for point in keypoints_3d)
        max_z = max(point[2] for point in keypoints_3d)

        # Set margin to provide some space around the keypoints
        margin = 50
